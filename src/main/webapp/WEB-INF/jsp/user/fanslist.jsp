...
<title>${user.nickname}的粉丝列表</title>
<link rel="stylesheet"
...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath }/css/follow.css">
	...
	<div class="container">
		<div class="row" align="center">
			<c:forEach items="${fansList }" var="fans">
				<div class="col-md-4" style="height: 150px; background-color: #fff; border: 7px solid #d6d6d6">
					<div class="leftbox">
						<!-- 头像 -->
						<div
					...
					<div class="rightbox" style="" align="center">
						<table>
							<tr>
								<td>
								<!-- 非本人 -->
								<c:if test="${fans.userId!=me.userId}">
									<!-- 相互关注 --> 
                                    <c:if test="${fans.relation.state==4}">
										<button id="${fans.userId }" class="btn btn-warning"
											onclick="javascript:ex_follow2(${fans.userId });">相互关注</button>
									</c:if> 
                                    <!-- 我关注他 --> 
                                    <c:if test="${fans.relation.state==3}">
										<button id="${fans.userId }" class="btn btn-success"
											onclick="javascript:ex_follow1(${fans.userId });">已关注</button>
									</c:if> 
                                    <!-- 他关注我 --> 
                                    <c:if test="${fans.relation.state==2}">
										<button id="${fans.userId }" class="btn btn-primary"
											onclick="javascript:ex_follow2(${fans.userId })">关注</button>
									</c:if>
                                    <!-- 陌生 -->
                                    <c:if test="${fans.relation.state==1}">
                                        <button id="${fans.userId }" class="btn btn-lg btn-primary"
                                            onclick="javascript:ex_follow1(${fans.userId})">关注</button>
                                    </c:if>
                                </c:if>
                                <!-- 本人 -->
                                 <c:if test="${fans.userId==me.userId}">
							...
							<tr>
								<td><span style="font-size: 17px">${fans.nickname }</span></td>
							</tr>
							<tr>
								<td><span style="font-size: 15px"> <!-- 性别 --> <c:if
											test="${fans.sex==0 }">♀</c:if> <c:if
											test="${fans.sex==1 }">♂</c:if> <!-- 年龄 -->
										${fans.age }岁 <!-- 所在省 --> ${fans.p } <!-- 所在市 -->
										${fans.c }
								</span></td>
							</tr>
						</table>
